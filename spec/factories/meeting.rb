...
FactoryGirl.define do
  ...
  factory :meeting do
    ...
    after(:create) do |meeting, evaluator|
      ...
      FactoryGirl.create(:meeting_subscription, meeting: meeting, user: user, created_by: user)
    end
      ...
      after(:create) do |meeting ,evaluator|
        2.times do
          ...
          if ([true, false].sample)
            subtopic = FactoryGirl.create(:subtopic, user: meeting.organizer)
            ContentAssociation.create :parent => topic, :child => subtopic
          end
          parent = [topic, subtopic].compact.sample

          2.times do
            note = FactoryGirl.create(:note, user: meeting.organizer)
            ContentAssociation.create :parent => parent, :child => note
          end

          2.times do
            decision = FactoryGirl.create(:decision, user: meeting.organizer)
            ContentAssociation.create :parent => parent, :child => decision
          end

          2.times do
            task = FactoryGirl.build(:task, user: meeting.organizer)
